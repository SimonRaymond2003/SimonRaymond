...
the columns i will use are y(Survived), Pclass, Sec, Age, SibSp, Parch, Fare, Emarked as they are the easiest to work with. other possibilities include using the titles for names 
```{r}
library(dplyr)
...
Firts i am going to predict the NAs for Age with a simple Random Forest model.
```{r}
...
# Initial Split
  ...
  # Predicting on the test set
  phat <- predict(model, test, type = "response")
